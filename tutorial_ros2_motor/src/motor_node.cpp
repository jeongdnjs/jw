    ...
    : Node("tutorial_ros2_motor"), count_(0)
{
  ...
  robot_command_subscription_ = this->create_subscription<std_msgs::msg::String>(
        "robot_command", 10, std::bind(&RosCommunicator::RobotCommandCallback, this, std::placeholders::_1));
}
...
void RosCommunicator::RobotCommandCallback(const std_msgs::msg::String::SharedPtr msg)
{
    ...
    if (command == "left")
    {
      MotorController(1, false, 50); // 1번 모터 역방향
      MotorController(2, false, 50);  // 2번 모터 정방향

    }
    else if (command == "right")
    {
      MotorController(1, true, 50);  // 1번 모터 정방향
      MotorController(2, true, 50); // 2번 모터 역방향

    }
    else if (command == "go")
    {
      MotorController(1, true, 50);
      MotorController(2, false, 50);
    }
